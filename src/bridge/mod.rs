...
use arrow::pyarrow::FromPyArrow; // This must be in scope
use arrow::ffi_stream::ArrowArrayStreamReader;
// We need the raw C structs from both sides
use arrow::ffi::{FFI_ArrowArray as ArrowArray, FFI_ArrowSchema as ArrowSchema};
use polars_arrow::ffi;
use arrow::ffi_stream::FFI_ArrowArrayStream;
use crate::engine::conditional_join::{execute_join, Condition};
...
fn extract_batches(obj: &Bound<'_, PyAny>) -> PyResult<Vec<RecordBatch>> {
    // 1. Manually call the capsule method
    let capsule = obj.call_method0("__arrow_c_stream__")?;

    // 2. Extract the raw pointer from the PyCapsule
    // A PyCapsule "pointer" name for Arrow streams is "arrow_array_stream"
    let stream_ptr = unsafe {
        let ptr = pyo3::ffi::PyCapsule_GetPointer(capsule.as_ptr(), "arrow_array_stream\0".as_ptr() as *const _);
        ptr as *mut FFI_ArrowArrayStream
    };

    // 3. Import from the raw pointer
    let reader = unsafe {
...
fn bridge_to_polars(batches: Vec<RecordBatch>) -> PyResult<DataFrame> {
    if batches.is_empty() {
...
fn bridge_to_arrow_rs(df: DataFrame) -> PyResult<Vec<RecordBatch>> {
    if df.height() == 0 {
        return Ok(vec![]);
    }
    ...
    println!("Output have {} chuncks", num_chunks);
    ...
    for chunk_idx in 0..num_chunks {
        ...
        println!("Total of {}", arrow_columns.len());
